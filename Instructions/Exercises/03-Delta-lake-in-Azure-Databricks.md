...
> **ヒント**: Azure Databricks ワークスペースに 13.3 LTS 以降のランタイム バージョンを備えたクラスターが既にある場合は、それを使ってこの演習を完了し、この手順をスキップできます。
...
1. ノートブックの最初のセルに次のコードを入力します。このコードでは、"*シェル*" コマンドを使用して、GitHub のデータファイルが、クラスターで使用される Databricks ファイル システム (DBFS) にダウンロードされます。
