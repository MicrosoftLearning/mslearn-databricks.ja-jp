...
lab:
  ...
  description: 順方向伝搬とアクティブ化関数を使って多層ネットワーク アーキテクチャを定義し、損失関数、オプティマイザー、複数のエポックに対する逆伝搬法を使ってそれをトレーニングすることで、PyTorch を用いたディープ ニューラル ネットワークの構築を実践的に経験します。 バッチ処理用の PyTorch データ ローダーの作成、オーバーフィットを防ぐためのトレーニングと検証の損失の監視、トレーニング メトリックの視覚化、新しい観測での推論のためのトレーニング済みモデルの保存と読み込みを行う方法を学びます。
  duration: 45 minutes
  level: 400
  islab: true
  primarytopics:
