...
lab:
  ...
  description: ユーザー グループのメンバーシップに基づいて機密データへのアクセスを制御する行フィルターと列マスクを使って、Unity Catalog にきめ細かなデータ セキュリティを実装する方法を学びます。 列の値をリダクトして行をフィルター処理する SQL 関数を作成し、それをテーブルに適用して、ユーザーのアクセス許可に基づいてデータの可視性を自動的に調整する動的ビューを作成する方法を練習します。 また、セキュリティ保護されたビューへのアクセスを許可し、異なるユーザー グループに対してセキュリティ ポリシーが正しく機能することを検証する方法も調べます。
  duration: 45 minutes
  level: 300
  islab: true
  primarytopics:
