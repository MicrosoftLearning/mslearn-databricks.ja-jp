...
lab:
  ...
  description: CSV ファイルの読み込みや、製品カテゴリ別の売上のグループ化のような集計の実行など、データを取り込んで処理するノートブックを実行する Databricks ジョブを作成して構成することで、データ ワークフローの自動化を実践的に経験します。 ジョブ インターフェイスを使って自動化されたタスクの設定、ジョブ実行の手動トリガー、その実行の監視を行い、スケーラブルで運用対応のデータ処理パイプラインを構築する方法を学びます。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
