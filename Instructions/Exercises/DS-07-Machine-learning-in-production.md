...
lab:
  ...
  description: 特徴エンジニアリングとモデル トレーニングのステージをカプセル化する ML パイプラインを構築し、MLflow のモデル レジストリを使ってトレーニング済みモデルを登録することで、完全な機械学習ライフサイクルを管理する実践経験を得ます。 登録されたモデルを運用対応のリアルタイム サービング エンドポイントとしてデプロイし、JSON 要求を送信して新しい所見の予測を生成してそれをテストする方法を学びます。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
