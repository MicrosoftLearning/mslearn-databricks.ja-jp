...
lab:
  ...
  description: 自動スキーマ進化とスキーマ変更処理を備えた効率的な増分データ インジェストのために、自動ローダーを使ってデータ パイプラインを最適化する、実践的な経験を得ます。 パーティション間により均等にデータを分散させてデータ スキューを軽減するためのソルト化や、ファイル圧縮と Z 順序付けクラスタリングによってクエリ パフォーマンスを向上させるための Delta Lake 最適化コマンドの使用など、実用的な最適化手法について学びます。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
