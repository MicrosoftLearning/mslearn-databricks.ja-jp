...
lab:
  ...
  description: 性別の含意がない入力プロンプトとある入力プロンプトを作成し、出力を比較してトレーニング データから受け継がれた潜在的なバイアスを検出することで、バイアスに関する大規模言語モデルのテストを実践的に体験します。 AI システムでバイアスを特定して分析するための実用的な手法を学び、AI のデプロイをいっそう公平で責任あるものにするための、トレーニング データの再サンプリング、再重み付け、変更などの軽減アプローチを理解します。
  duration: 20 minutes
  level: 400
  islab: true
  primarytopics:
