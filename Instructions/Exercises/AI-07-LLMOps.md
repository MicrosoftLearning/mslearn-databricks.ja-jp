...
lab:
  ...
  description: MLflow を使って、各モデル実行のパラメーター、メトリック、予測、成果物などの LLM の相互作用を追跡してログすることで、LLMOps プラクティスを実装する実践的な経験を得ます。 MLflow の自動ログ機能と Trace UI を使って、モデルのパフォーマンスを経時的に監視し、異なる実行の間でトレースを比較し、データ ドリフトを検出して、LLM アプリケーションが運用環境で信頼性とパフォーマンスを維持できるようにする方法を学びます。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
