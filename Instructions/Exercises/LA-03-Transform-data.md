...
1. 既定のノートブック名 (**無題のノートブック *[日付]***) を `Transform data with Spark` に変更し、**[接続]** ドロップダウン リストで、**[サーバーレス SQL ウェアハウス]** がまだ選択されていない場合は選択します。 コンピューティングが実行されていない場合は、開始に 1 分ほどかかる場合があります。
