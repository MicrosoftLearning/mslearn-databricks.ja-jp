...
lab:
  ...
  description: カスタム評価基準 (ガイドラインと安全性スコアラー) を定義し、テスト データセットに対して体系的な評価を実行することで、MLflow の評価フレームワークを使って LLM 出力を評価する実践的な経験を得ます。 評価結果を分析し、創造性や安全性などの特定の基準に基づいてシステム プロンプトを調整し、評価実行を比較して改善を検証することで、モデルのパフォーマンスを反復的に向上させる方法を学びます。
  duration: 30 minutes
  level: 400
  islab: true
  primarytopics:
...
1. ノートブックに名前を付け、言語として [`Python`] を選択します。 既定のコンピューティングとして **[サーバーレス]** を選択します。
